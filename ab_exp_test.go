...
func TestABCoreEvalExperimentPublic(t *testing.T) {
	...
	t.Run("variant-1", func(t *testing.T) {
		result, err := core.evalAB(ABUser{LoginID: variant1User}, spec, 0)
		require.NoError(t, err)
	...
	t.Run("variant-2", func(t *testing.T) {
		result, err := core.evalAB(ABUser{LoginID: variant2User}, spec, 0)
		require.NoError(t, err)
		require.NotNil(t, result.VariantID)
...
func TestABCoreEvalExperimentTrafficRollout(t *testing.T) {
	...
	t.Run("in-traffic", func(t *testing.T) {
		result, err := core.evalAB(ABUser{LoginID: userInTraffic}, spec, 0)
		require.NoError(t, err)
		require.NotNil(t, result.VariantID)
	...
	t.Run("out-traffic", func(t *testing.T) {
		result, err := core.evalAB(ABUser{LoginID: userOutTraffic}, spec, 0)
		require.NoError(t, err)
...
func TestABCoreEvalExperimentTarget(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			user:          ABUser{LoginID: "user1", Props: Properties{"$app_version": "9.0"}},
			shouldBeInExp: false,
			wantVariantID: nil,
		},
		{
			name:          "missing-version",
			user:          ABUser{LoginID: "user1"},
			shouldBeInExp: false,
			wantVariantID: nil,
		},
		{
			name:          "different-version",
			user:          ABUser{LoginID: "user1", Props: Properties{"$app_version": "10.1"}},
			shouldBeInExp: false,
			wantVariantID: nil,
		},
	}
	...
	t.Run("variant-distribution", func(t *testing.T) {
		...
		var variant1User, variant2User string
		for _, uid := range testLoginIDs {
			result, err := core.evalAB(ABUser{LoginID: uid, Props: Properties{"$app_version": "10.0"}}, spec, 0)
			require.NoError(t, err)
			if result.VariantID != nil {
				if *result.VariantID == "v1" && variant1User == "" {
					variant1User = uid
		...
		result1, err := core.evalAB(ABUser{LoginID: variant1User, Props: Properties{"$app_version": "10.0"}}, spec, 0)
		require.NoError(t, err)
		require.Equal(t, float64(0), result1.GetNumber("test", -1))

		// Verify variant 2 payload
		result2, err := core.evalAB(ABUser{LoginID: variant2User, Props: Properties{"$app_version": "10.0"}}, spec, 0)
		require.NoError(t, err)
...
func TestABCoreEvalExperimentLayer(t *testing.T) {
	...
	t.Run("traffic-allocation", func(t *testing.T) {
		...
		result1, err := core.evalAB(ABUser{LoginID: userInTraffic}, spec, 0)
		require.NoError(t, err)
		...
		result2, err := core.evalAB(ABUser{LoginID: userOutTraffic}, spec, 0)
		require.NoError(t, err)
	...
	t.Run("variant-distribution-and-payloads", func(t *testing.T) {
		...
		// Verify variant 1 payload
		result1, err := core.evalAB(ABUser{LoginID: variant1User}, spec, 0)
		require.NoError(t, err)
		require.NotNil(t, result1.VariantID)
		...
		// Verify variant 2 payload
		result2, err := core.evalAB(ABUser{LoginID: variant2User}, spec, 0)
		require.NoError(t, err)
...
func TestABCoreEvalExperimentSticky(t *testing.T) {
	...
	t.Run("use-sticky-cache", func(t *testing.T) {
		...
		result, err := core.evalAB(ABUser{LoginID: "sticky-user-cache", Props: Properties{"is_member": false}}, spec, 0)
		require.NoError(t, err)
		require.NotNil(t, result.VariantID)
		require.Equal(t, "v2", *result.VariantID)
...
func TestABCoreEvalExperimentGateTarget(t *testing.T) {
	...
	t.Run("gate-fails-when-dependency-fails", func(t *testing.T) {
		result, err := core.evalAB(ABUser{LoginID: "user0", Props: Properties{"$app_version": "9.9"}}, exp, 0)
		require.NoError(t, err)
		require.Nil(t, result.VariantID)
	})
	...
	t.Run("gate-passes-when-dependency-passes", func(t *testing.T) {
		...
		for i := 0; i < 200; i++ {
			...
			if res, err := core.evalAB(ABUser{LoginID: uid, Props: Properties{"$app_version": "10.1"}}, exp, 0); err == nil && res.VariantID != nil {
				passingUser = uid
				break
			}
		}
		...
		result, err := core.evalAB(ABUser{LoginID: passingUser, Props: Properties{"$app_version": "10.1"}}, exp, 0)
		require.NoError(t, err)
		require.NotNil(t, result.VariantID)
		require.Contains(t, []string{"v1", "v2"}, *result.VariantID)
	})
...
func TestABCoreEvalExperimentGateTargetFail(t *testing.T) {
	...
	t.Run("gate-fail-when-dependency-passes", func(t *testing.T) {
		...
		for i := 0; i < 200; i++ {
			uid := fmt.Sprintf("user-pass-%d", i)
			depRes, err := core.evalAB(ABUser{LoginID: uid, Props: Properties{"$app_version": "10.1"}}, dep, 0)
			require.NoError(t, err)
